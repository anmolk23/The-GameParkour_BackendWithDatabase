...
// -------------------- RENDER SQLite PERSISTENT PATH --------------------
const DB_PATH = process.env.DB_PATH || path.join(__dirname, "gamerverse.db");
...
app.get("/api/profile", requireLogin, (req, res) => {
  db.get(
    `SELECT id, name, email, favorite_genre, hours_played, wins, losses, 
            level, photo, bio 
     FROM users WHERE id = ?`,
    [req.session.userId],
    (err, row) => {
...
app.post("/api/collection", requireLogin, (req, res) => {
  ...
  db.run(
    `INSERT INTO games (user_id, title, genre, hours, owned, img_url)
     VALUES (?, ?, ?, ?, 1, ?)`,
    [req.session.userId, title, genre, hours || 0, img_url || ""],
    function (err) {
      if (err) return res.status(500).json({ error: "DB error" });
...
app.get("/api/wishlist", requireLogin, (req, res) => {
  db.all(
    ...
    (err, rows) => {
      ...
      // IMPORTANT: wishlist must return key named "wishlist"
